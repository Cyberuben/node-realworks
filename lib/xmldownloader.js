...
class XMLDownloader {
	...
	download(type) {
		return new Promise((resolve, reject) => {
			...
			if(this._options.version && typeof this._options.version == "object") {
				if(this._options.version.hasOwnProperty(type)) {
					queryParams.versie = this._options.version[type];
				}
			}
			...
			const options = {
				...
				path: (!this._options.override || !this._options.override.path ? this._path + "?" + qs.stringify(queryParams) : this._options.override.path.replace("{type}", type))
			};
			...
			const request = https.request(options, (res) => {
				...
				.then((filename) => {
					this._parent.emit("zip downloaded", type, filename);
					return this._unpackZip(filename)
				})
				.then((xmlFilename) => {
					this._parent.emit("zip extracted", type, xmlFilename);
					return this._readXml(xmlFilename);
				})
				.then((xmlData) => {
					this._parent.emit("xml read", type, xmlData);
					resolve(xmlData);
				})
				.catch((err) => {
					reject(err);
